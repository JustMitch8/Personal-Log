...
async function fetchData() {
  ...
  // Retry with increasing delays to handle JWT clock skew
  let authErr;
  for (const delay of [2000, 5000, 10000, 15000, 20000]) {
...
// ── Main ───────────────────────────────────────────────────────────
async function main() {
  // Guard: only run between 6:30PM and 7:30PM Melbourne time
  // Prevents double-firing during DST transition weeks
  const now = new Date();
  const tz  = 'Australia/Melbourne';
  const hour   = +new Intl.DateTimeFormat('en-AU', { timeZone: tz, hour:   'numeric', hour12: false }).format(now);
  const minute = +new Intl.DateTimeFormat('en-AU', { timeZone: tz, minute: 'numeric' }).format(now);
  const totalMins = hour * 60 + minute;
  if (totalMins < 18 * 60 + 30 || totalMins > 19 * 60 + 30) {
    console.log(`Melbourne time is ${hour}:${String(minute).padStart(2,'0')} — outside 6:30–7:30PM window, skipping.`);
    return;
  }
  console.log(`Melbourne time is ${hour}:${String(minute).padStart(2,'0')} — within window, proceeding.`);
